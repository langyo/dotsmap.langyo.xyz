...
const paletteLabCache = new Map<string, [number, number, number]>()

function getPaletteLab(color: BeadColor): [number, number, number] {
  const cached = paletteLabCache.get(color.code)
  if (cached) return cached
  const lab = rgbToLab(color.r, color.g, color.b)
  paletteLabCache.set(color.code, lab)
  return lab
}
...
export function findClosestColor(r: number, g: number, b: number, palette: BeadColor[]): BeadColor {
  ...
  for (const color of palette) {
    const [cl, ca, cb] = getPaletteLab(color)
    const dl = l - cl
    const da = a - ca
    const db = bl - cb
